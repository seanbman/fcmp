...
// Its JSON representation is deliberately not this struct layout; MarshalJSON
// and UnmarshalJSON expose the stable v1 wire envelope {v,type,payload,...}.
...
func (d Dispatch) MarshalJSON() ([]byte, error) {
	...
	if !knownFunction(d.Function) {
		return nil, fmt.Errorf("neith: unknown protocol type %q", d.Function)
	}

	payload, err := json.Marshal(d.payload())
	if err != nil {
		return nil, fmt.Errorf("neith: encode %s payload: %w", d.Function, err)
	}
	return json.Marshal(protocolEnvelope{
		Version:   version,
		Type:      d.Function,
		ID:        d.ID,
		Key:       d.Key,
...
func (d *Dispatch) UnmarshalJSON(data []byte) error {
	...
	if err := decodeJSONStrict(data, &envelope); err != nil {
		return fmt.Errorf("neith: invalid protocol envelope: %w", err)
	}
	if envelope.Version != ProtocolVersion {
		return fmt.Errorf("neith: unsupported protocol version %d", envelope.Version)
	}
	if !knownFunction(envelope.Type) {
		return fmt.Errorf("neith: unknown protocol type %q", envelope.Type)
	}
	if len(envelope.Payload) == 0 || bytes.Equal(envelope.Payload, []byte("null")) {
		return errors.New("neith: protocol payload is required")
	}
	...
	*d = Dispatch{
	...
	if err := d.decodePayload(envelope.Payload); err != nil {
		return fmt.Errorf("neith: invalid %s payload: %w", envelope.Type, err)
	}
	return nil
}
...
func (d *Dispatch) decodePayload(raw json.RawMessage) error {
	switch d.Function {
	case ping:
		return decodeJSONStrict(raw, &d.FnPing)
	case render:
		return decodeJSONStrict(raw, &d.FnRender)
	case class:
		return decodeJSONStrict(raw, &d.FnClass)
	case dom:
		return decodeJSONStrict(raw, &d.FnDOM)
	case redirect:
		return decodeJSONStrict(raw, &d.FnRedirect)
	case event:
		return decodeJSONStrict(raw, &d.FnEvent)
	case custom:
		return decodeJSONStrict(raw, &d.FnCustom)
	case fnError:
		return decodeJSONStrict(raw, &d.FnError)
	default:
		return fmt.Errorf("unknown protocol type %q", d.Function)
	}
}
...
func knownFunction(fn functionName) bool {
	switch fn {
	case ping, render, class, dom, redirect, event, custom, fnError:
		return true
	default:
		return false
	}
